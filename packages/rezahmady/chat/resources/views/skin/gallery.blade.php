<div class="{{$class}}" x-data="Gallery()" x-init="modal = false;slideIndex = 1;">
    <div class="gallery-thumb-holder">
        ...
        <div class="gallery-column gallery-thumb">
          <img src="{{asset(config('rezahmady.chat.uploud_photo_path').$photo)}}" style="width:100%" x-on:click="openModal();currentSlide({{$key+1}},'{{$id}}')" class="hover-shadow cursor">
        </div>
        @endforeach
    </div>
        ...
        <div class="modal-content">
            @foreach ($photos as $key => $photo)
                <div class="mySlides mySlides{{$id}}">
                    <a href="{{asset(config('rezahmady.chat.uploud_photo_path').$photo)}}" download class="gallery-download"><i class="fa fa-download"></i></a>
                    <div class="numbertext">{{$key+1}} / {{$total}}</div>
                    <img src="{{asset(config('rezahmady.chat.uploud_photo_path').$photo)}}" >
                </div>
            @endforeach
                        ...
                        <li class="list-inline-item">
                            <a href="#">
                                <figure class="avatar avatar-lg">
                                    <img class="demo demo{{$id}} cursor" src="{{asset(config('rezahmady.chat.uploud_photo_path').$photo)}}" x-on:click="currentSlide({{$key+1}},'{{$id}}')">
                                </figure>
                            </a>
                        </li>
                    @endforeach
