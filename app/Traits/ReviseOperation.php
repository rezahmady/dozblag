...
trait ReviseOperation
{
    ...
    public function restoreRevision($id)
    {
        ...
        if (! $revisionId) {
            ...
            $old_values = array_map('unserialize',
                array_diff(array_map('serialize', json_decode($revision->old_value, true)), array_map('serialize', json_decode($revision->new_value, true))));
            if($revision->key === 'extras') {
//            dd($old_values,$revision->key);
                $new_values = [];
                foreach ($old_values as $key => $value) {
                    $value = (is_array($value)) ? json_encode($value) : $value;
                    $new_values['extras->'.$key] = $value;
                }
                $entry->forceFill($new_values)->save();
            } else {
