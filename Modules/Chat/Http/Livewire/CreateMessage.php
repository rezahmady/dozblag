...
use Modules\Chat\Events\MessageAdded;
use Livewire\WithFileUploads;
use Illuminate\Support\Facades\Storage;
use Modules\Chat\Events\RoomStarted;
...
class CreateMessage extends Component
{
    ...
    public function saveVoice($voice)
    {
        ...
        $destination = config('chat.upload_voice_path').$voice;
    ...
    public function savePhotos()
    {
        ...
        foreach ($this->photos as $photo) {
            ...
            array_push($filenames, config('chat.upload_photo_path').$photo->getFilename());
            $resource = "/livewire/preview-file/{$photo->getFilename()}";
            $destination = config('chat.upload_photo_path').$photo->getFilename();
            if(Storage::disk('local')->exists($resource)) Storage::disk('local')->move($resource, $destination);
        }
    ...
    {
        return view('chat::skin.create-message');
    }
}
