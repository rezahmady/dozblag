...
class PageCrudController extends CrudController
{
    ...
    public function setup()
    {
        ...
        $this->crud->setEntityNameStrings(trans('page::page.page'), trans('page::page.pages'));
    ...
    protected function setupListOperation()
    {
        ...
        $this->crud->addButton('top', 'page_create_grouped', 'view', 'page::buttons.page_create_grouped', 'beginning');
        
        $this->crud->addColumn([
            'name' => 'name',
            'label' => trans('page::page.name'),
        ]);
        $this->crud->addColumn([
            'name' => 'template',
            'label' => trans('page::page.template'),
            'type' => 'closure',
            'function' => function($entry) {
                return get_page_templates_array()[$entry->template];
            },
        ]);
        $this->crud->addColumn([
            'name' => 'slug',
            'label' => trans('page::page.slug'),
        ]);
    ...
    {
        Widget::add()->type('script')->content(asset('modules/page/js/page_create_crud.js'));
        // Note:
        ...
        $this->crud->addField([
            'name' => 'title',
            'label' => trans('page::page.page_title'),
            'type' => 'text',
            'wrapperAttributes' => [
                'class' => 'form-group col-md-6',
            ],
        ...
        // Add modules page fields
        $this->modulesOptions($template);
    ...
    public function addDefaultPageFields($template = false)
    {
        $this->crud->addField([
            ...
            'options' => get_page_templates_array(),
            'value' => $template,
            'allows_null' => false,
            'wrapperAttributes' => [
                'class' => 'form-group col-md-6',
        ...
        $this->crud->addField([
            'name' => 'slug',
            'label' => trans('page::page.page_slug'),
            'type' => 'text',
            'hint' => trans('page::page.page_slug_hint'),
            'wrapperAttributes' => [
                'class' => 'form-group col-md-6',
            ],
            'tab' => 'تنظیمات',
    ...
    public function useTemplate($template_name = false)
    {
        ...
        if ($template_name and method_exists($this, $template_name)) {
            $this->{$template_name}();
        }
    }
    ...
    {
        if(defined('THEME_FOLDER') and THEME_FOLDER != 'default') {
            $options = resolve("Themes\\".THEME_FOLDER."\\ThemeOptions");
    ...
    /**
     * Add the fields defined for a specific module.
     *
     * @param  string $theme_folder The name of the template folder that should be used in the current form.
     */
    public function modulesOptions($template)
    {
        Hook::action('modules.page.action.fields.page_'.$template, $this->crud);
    }
}