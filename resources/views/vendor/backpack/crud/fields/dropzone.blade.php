        	...
        	@foreach($field['value'] as $key => $file_path)
        		<div class="dz-preview dz-image-preview dz-complete text-center">
                    ...
                    <div class="dz-image-no-hover">
                        ...
                        @else
                            <a href="{{ config('filesystems.disks.'.$field['disk'].'.url') .'/'. $field['destination_path'] .'/'. $field['thumb_prefix'] . basename($file_path) }}" data-toggle="modal" data-target="#imageModal-{{ $key }}">
                                <img src="{{ config('filesystems.disks.'.$field['disk'].'.url') .'/'. $field['destination_path'] .'/'. $field['thumb_prefix'] . basename($file_path) }}" class="img-thumbnail" />
                            </a>
                        @endif
                    </div>
                    <a class="dz-remove dz-remove-existing" href="javascript:undefined;" data-path="{{ basename ($file_path) }}">
                        {{ trans('backpack::dropzone.remove_file') }}
                    </a>
                </div>
            @endforeach
        @endif
...
@if ($crud->fieldTypeNotLoaded($field))
                    ...
                    <div class="modal fade" id="imageModal-{{ $key }}" tabindex="-1" role="dialog" aria-labelledby="imageModalLabel-{{ $key }}" aria-hidden="true">
                        <div class="modal-dialog modal-lg" role="document">
                            <div class="modal-content">
                                ...
                                <div class="modal-body text-center">
                                    <img src="{{ config('filesystems.disks.'.$field['disk'].'.url') .'/'. $field['destination_path'] .'/'. $field['thumb_prefix'] . basename($file_path) }}" class="img-fluid" />
                                    <a href="{{ config('filesystems.disks.'.$field['disk'].'.url') .'/'. $field['destination_path'] .'/'. $field['thumb_prefix'] . basename($file_path) }}" class="btn btn-primary mt-2" download>دانلود</a>
                                    <button onclick="printImage('{{ config('filesystems.disks.'.$field['disk'].'.url') .'/'. $field['destination_path'] .'/'. $field['thumb_prefix'] . basename($file_path) }}')" class="btn btn-secondary mt-2">پرینت</button>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        ...
        $("div#{{ $field['name'] }}-dropzone").dropzone({
            ...
            url: "{{ route( Str::kebab(class_basename($crud->model)) . '.dropzone-add') }}",

            acceptedFiles: "{{ $field['mimes'] ?? 'image/*' }}",
            maxFilesize: {{ $field['max_file_size'] ?? 3 }},
            maxFiles: {{ $field['max_file'] ?? 3 }},
            headers: {
        ...
        $('.dz-remove-existing').click(function (e) {
            ...
            $.ajax({
                url: "{{ route( Str::kebab(class_basename($crud->model)) . '.dropzone-remove') }}",
                type: 'POST',
                data: {
                    {{--entry           : {{ $crud->entry->id ?? '' }},--}}
                    disk            : "{{ $field['disk'] }}",
